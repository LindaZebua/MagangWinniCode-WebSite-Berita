                ...
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-body card-block">
                            <form action="{{ route('media.update', $media->media_id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                                @csrf
                                @method('PUT')
                                <div class="row form-group">
                                    ...
                                    <div class="col-12 col-md-9">
                                        ...
                                        @enderror
                                        @if ($media->file_path)
                                            <small class="form-text text-muted">File saat ini: <a href="{{ asset('storage/' . $media->file_path) }}" target="_blank">{{ $media->file_path }}</a></small>
                                        @endif
                                    </div>
                                </div>
                                <div class="row form-group">
                                    ...
                                    <div class="col-12 col-md-9">
                                        <select name="news_id" id="news_id" class="form-control @error('news_id') is-invalid @enderror">
                                            ...
                                            @foreach ($news as $item)
                                                <option value="{{ $item->news_id }}" {{ old('news_id', $media->news_id) == $item->news_id ? 'selected' : '' }}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                        @error('news_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
